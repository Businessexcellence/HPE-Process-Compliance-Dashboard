...
<div class="content">
  ...
  <div class="tab-content" id="tab-data">
    ...
    <div class="upload-zone" id="uploadZone" onclick="document.getElementById('fileInput').click()"
      ...
      <div class="upload-subtitle">Supports .xlsx, .xls, .csv &nbsp;|&nbsp; Max 50 MB<br>
        <strong>Required columns:</strong> Month, Week, Parameter, Result (Pass/Fail/NA)<br>
        <strong>Optional columns:</strong> Hire_Type (HPE_Experienced / HPE_UR), Recruiter, PM, Stage, Criticality
      </div>
    </div>
    ...
    <div class="card card-full" id="dmHireTypeCard">
      ...
      <div id="dmHireTypeContent" style="margin-top:12px">
        <div style="padding:28px;text-align:center;color:var(--text-muted)">
          ...
          <div style="font-size:12px">Upload a file with a <strong>Hire_Type</strong> column containing <strong>HPE_Experienced</strong> or <strong>HPE_UR</strong> values to see the split accuracy analysis here and in the Executive Summary.</div>
        </div>
      </div>
    </div>
    ...
    <!-- Column Format Guide -->
    <div class="card card-full">
      <div class="card-title"><i class="fas fa-table"></i> Expected File Format</div>
      <div class="card-subtitle">Your file should contain these columns (header row in row 1)</div>
      <div style="overflow-x:auto;margin-top:12px">
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          <tbody>
            <tr style="background:var(--surface)"><td style="padding:7px 12px;font-weight:700;color:var(--hpe-green)">Month</td><td style="padding:7px 12px"><span style="background:#e6f9f5;color:#01A982;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Required</span></td><td style="padding:7px 12px;color:var(--text-muted)">Jan, Feb, Mar, Apr</td><td style="padding:7px 12px">Month of audit</td></tr>
            ...
            <tr style="background:var(--surface-2)"><td style="padding:7px 12px;font-weight:700;color:var(--hpe-green)">Result</td><td style="padding:7px 12px"><span style="background:#e6f9f5;color:#01A982;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Required</span></td><td style="padding:7px 12px;color:var(--text-muted)">Pass, Fail, NA</td><td style="padding:7px 12px">Audit outcome</td></tr>
            <tr style="background:var(--surface)"><td style="padding:7px 12px;font-weight:700;color:#FF8300">Hire_Type</td><td style="padding:7px 12px"><span style="background:#fff3e6;color:#FF8300;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Optional</span></td><td style="padding:7px 12px;color:var(--text-muted)"><strong>HPE_Experienced</strong> or <strong>HPE_UR</strong></td><td style="padding:7px 12px">Enables split accuracy by hire type</td></tr>
            <tr style="background:var(--surface-2)"><td style="padding:7px 12px;font-weight:700;color:#FF8300">Recruiter</td><td style="padding:7px 12px"><span style="background:#fff3e6;color:#FF8300;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Optional</span></td><td style="padding:7px 12px;color:var(--text-muted)">Recruiter full name</td><td style="padding:7px 12px">Recruiter performance panel</td></tr>
            <tr style="background:var(--surface)"><td style="padding:7px 12px;font-weight:700;color:#FF8300">PM</td><td style="padding:7px 12px"><span style="background:#fff3e6;color:#FF8300;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Optional</span></td><td style="padding:7px 12px;color:var(--text-muted)">Program Manager name</td><td style="padding:7px 12px">PM panel drill-down</td></tr>
            <tr style="background:var(--surface-2)"><td style="padding:7px 12px;font-weight:700;color:#FF8300">Stage</td><td style="padding:7px 12px"><span style="background:#fff3e6;color:#FF8300;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Optional</span></td><td style="padding:7px 12px;color:var(--text-muted)">Pre Selection, Post Selection</td><td style="padding:7px 12px">Stage accuracy charts</td></tr>
            <tr style="background:var(--surface)"><td style="padding:7px 12px;font-weight:700;color:#FF8300">Criticality</td><td style="padding:7px 12px"><span style="background:#fff3e6;color:#FF8300;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">Optional</span></td><td style="padding:7px 12px;color:var(--text-muted)">Critical, Non Critical</td><td style="padding:7px 12px">Critical parameter gauges</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
function initTrendCharts() {
  ...
  charts['stageComparisonChart'] = new Chart(scCtx, {
    ...
    data: {
      ...
      datasets: [
        { label: 'Pass %', data: [+(5962/(5962+92+68)*100).toFixed(1), +(2438/(2438+36+3)*100).toFixed(1)], backgroundColor: '#01A982' },
        { label: 'Fail %', data: [+(92/(5962+92+68)*100).toFixed(1), +(36/(2438+36+3)*100).toFixed(1)], backgroundColor: '#C54E4B' },
        { label: 'N/A %', data: [+(68/(5962+92+68)*100).toFixed(1), +(3/(2438+36+3)*100).toFixed(1)], backgroundColor: '#94a3b8' }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      ...
      scales: {
        ...
        y: { stacked: true, min: 0, max: 100, ticks: { callback: v => v + '%', font:{size:11} }, title: { display: true, text: '% of Audits' } }
      }
    }
  });
...
function _doParseFile(file) {
  ...
  reader.onload = function(e) {
    try {
      var rows = [];
      if (isCSV) {
        rows = _parseCSVRows(e.target.result);
      } else {
        // Excel via SheetJS
        var wb = XLSX.read(e.target.result, {type:'array'});
        // Try "Parameter Data" sheet first, then fall back to first sheet
        var wsName = wb.SheetNames.includes('Parameter Data') ? 'Parameter Data' : wb.SheetNames[0];
        var ws = wb.Sheets[wsName];
        rows = XLSX.utils.sheet_to_json(ws, {defval:''});
      }

      if (!rows || rows.length === 0) {
        _uploadError(zone, 'No data rows found in the file.');
        return;
      }

      // Validate required columns
      var firstRow = rows[0];
      var keys = Object.keys(firstRow).map(k => k.trim());
      var required = ['Month', 'Week', 'Parameter', 'Result'];
      var missing = required.filter(function(r){
        return !keys.some(function(k){ return k.toLowerCase() === r.toLowerCase(); });
      });
      if (missing.length > 0) {
        _uploadError(zone, 'Missing required columns: ' + missing.join(', ') + '. Expected columns: Month, Week, Parameter, Result, Hire_Type (optional), Recruiter (optional), PM (optional), Stage (optional), Criticality (optional).');
        return;
      }

      // Normalise keys (trim + title-case matching)
      rows = rows.map(function(r) {
        var n = {};
        Object.keys(r).forEach(function(k){ n[k.trim()] = (typeof r[k] === 'string') ? r[k].trim() : r[k]; });
        return n;
      });

      // Process and inject into DASHBOARD_DATA
      var result = _processRows(rows);
      UPLOADED_DATA = result;

      // Inject into live DASHBOARD_DATA + PERF_DATA
      _injectDashboardData(result);

      // Rebuild all charts and panels
      _fullDashboardRebuild();

      // Update upload zone to success
      zone.innerHTML = \`
        ...
        <div class="upload-subtitle">\${rows.length.toLocaleString()} rows processed &nbsp;|&nbsp; Dashboard fully updated &nbsp;|&nbsp; Drop another file to refresh</div>
      \`;

      // Refresh status panel
      _updateDataStatusPanel(result, file);
      ...
      showToast('Dashboard updated from ' + file.name + ' (' + rows.length.toLocaleString() + ' rows)', 'success');
...
function _parseCSVRows(text) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    var vals = lines[i].split(',').map(v => v.replace(/^"|"$/g,'').trim());
    var obj = {};
...
// Expected columns (case-insensitive): Month, Week, Parameter, Result (Pass/Fail/NA),
//   Hire_Type (HPE_Experienced / HPE_UR), Recruiter, PM, Stage, Criticality
function _processRows(rows) {
  // Helper: find column value case-insensitively
  function col(row, name) {
    var keys = Object.keys(row);
    var k = keys.find(function(k){ return k.toLowerCase() === name.toLowerCase(); });
    return k ? (row[k] || '') : '';
  }
  ...
  // Month number map for sorting (Jan=10 ... Apr=13 per FY convention)
  ...
  var HIRE_TYPES = ['HPE_Experienced', 'HPE_UR'];

  // Classify result
  function classifyResult(r) {
    var v = (r||'').toString().trim().toLowerCase();
    if (v === 'pass' || v === '1' || v === 'true' || v === 'yes') return 'pass';
    if (v === 'na' || v === 'n/a' || v === 'not applicable') return 'na';
    return 'fail';
  }

  // Bucket rows
  var monthMap = {};   // key: Month
  var weekMap  = {};   // key: "Month||Week"
  var paramMap = {};   // key: Parameter
  var recruiterMap = {};
  var pmMap    = {};
  var stageMap = {};
  var critMap  = {};
  ...
  rows.forEach(function(row) {
    var month  = (col(row,'Month')||'').toString().trim();
    var week   = parseInt(col(row,'Week')) || 1;
    var param  = (col(row,'Parameter')||'').toString().trim();
    var res    = classifyResult(col(row,'Result'));
    var ht     = (col(row,'Hire_Type')||col(row,'HireType')||col(row,'hire_type')||'').toString().trim();
    var rec    = (col(row,'Recruiter')||'').toString().trim();
    var pm     = (col(row,'PM')||col(row,'Program Manager')||'').toString().trim();
    var stage  = (col(row,'Stage')||'').toString().trim();
    var crit   = (col(row,'Criticality')||'').toString().trim();

    // Normalise hire type
    if (!ht || (ht.toUpperCase() !== 'HPE_EXPERIENCED' && ht.toUpperCase() !== 'HPE_UR')) {
      ht = 'HPE_Experienced'; // default bucket
    }
    ht = ht.toUpperCase() === 'HPE_UR' ? 'HPE_UR' : 'HPE_Experienced';

    var isPass = res === 'pass', isNA = res === 'na', isFail = res === 'fail';
    // NA rows excluded from accuracy denominator
    var countable = !isNA;

    if (countable) { totalCount++; if (isPass) totalPass++; if (isFail) totalFail++; }
    totalNA += isNA ? 1 : 0;

    // -- Month aggregation --
    if (!monthMap[month]) monthMap[month] = {count:0,pass:0,fail:0,na:0};
    if (countable) { monthMap[month].count++; if (isPass) monthMap[month].pass++; if (isFail) monthMap[month].fail++; }
    monthMap[month].na += isNA ? 1 : 0;

    // -- Week aggregation --
    var wk = month + '||' + week;
    if (!weekMap[wk]) weekMap[wk] = {month,week,count:0,pass:0,fail:0,na:0};
    if (countable) { weekMap[wk].count++; if (isPass) weekMap[wk].pass++; if (isFail) weekMap[wk].fail++; }
    weekMap[wk].na += isNA ? 1 : 0;

    // -- Parameter aggregation --
    if (param) {
      if (!paramMap[param]) paramMap[param] = {count:0,pass:0,fail:0,na:0};
      if (countable) { paramMap[param].count++; if (isPass) paramMap[param].pass++; if (isFail) paramMap[param].fail++; }
      paramMap[param].na += isNA ? 1 : 0;
    }

    // -- Recruiter aggregation --
    if (rec) {
      if (!recruiterMap[rec]) recruiterMap[rec] = {count:0,pass:0,fail:0,na:0,pm:pm,months:{}};
      if (countable) { recruiterMap[rec].count++; if (isPass) recruiterMap[rec].pass++; if (isFail) recruiterMap[rec].fail++; }
      recruiterMap[rec].na += isNA ? 1 : 0;
      if (!recruiterMap[rec].months[month]) recruiterMap[rec].months[month] = {count:0,pass:0,fail:0};
      if (countable) { recruiterMap[rec].months[month].count++; if (isPass) recruiterMap[rec].months[month].pass++; if (isFail) recruiterMap[rec].months[month].fail++; }
      if (pm && !recruiterMap[rec].pm) recruiterMap[rec].pm = pm;
    }

    // -- PM aggregation --
    if (pm) {
      if (!pmMap[pm]) pmMap[pm] = {count:0,pass:0,fail:0,na:0,recruiters:new Set()};
      if (countable) { pmMap[pm].count++; if (isPass) pmMap[pm].pass++; if (isFail) pmMap[pm].fail++; }
      pmMap[pm].na += isNA ? 1 : 0;
      if (rec) pmMap[pm].recruiters.add(rec);
    }

    // -- Stage aggregation --
    if (stage) {
      if (!stageMap[stage]) stageMap[stage] = {count:0,pass:0,fail:0,na:0};
      if (countable) { stageMap[stage].count++; if (isPass) stageMap[stage].pass++; if (isFail) stageMap[stage].fail++; }
      stageMap[stage].na += isNA ? 1 : 0;
    }

    // -- Criticality aggregation --
    if (crit) {
      if (!critMap[crit]) critMap[crit] = {count:0,pass:0,fail:0,na:0};
      if (countable) { critMap[crit].count++; if (isPass) critMap[crit].pass++; if (isFail) critMap[crit].fail++; }
      critMap[crit].na += isNA ? 1 : 0;
    }

    // -- Hire Type month aggregation --
    var htMap = hireTypeMonthMap[ht];
    if (!htMap[month]) htMap[month] = {count:0,pass:0,fail:0,na:0};
    if (countable) { htMap[month].count++; if (isPass) htMap[month].pass++; if (isFail) htMap[month].fail++; }
    htMap[month].na += isNA ? 1 : 0;
    // combined
    if (!hireTypeMonthMap.combined[month]) hireTypeMonthMap.combined[month] = {count:0,pass:0,fail:0,na:0};
    if (countable) { hireTypeMonthMap.combined[month].count++; if (isPass) hireTypeMonthMap.combined[month].pass++; if (isFail) hireTypeMonthMap.combined[month].fail++; }
    hireTypeMonthMap.combined[month].na += isNA ? 1 : 0;

    // Hire type totals
    if (countable) { htTotals[ht].count++; if (isPass) htTotals[ht].pass++; if (isFail) htTotals[ht].fail++; }
    htTotals[ht].na += isNA ? 1 : 0;
  });
  ...
  var week_stats = Object.keys(weekMap).map(function(k) {
    var d = weekMap[k];
    var mn = monthNum(d.month);
    return {
      Month: d.month, Month_Number: mn, Week: d.week,
      Opportunity_Count: d.count, Opportunity_Pass: d.pass,
      Opportunity_Fail: d.fail, Opportunity_NA: d.na,
      Accuracy: calcAcc(d.pass, d.count),
  ...
  // Build recruiter_bottom (all recruiters, sorted by accuracy asc)
  ...
  var recruiter_bottom = Object.keys(recruiterMap).map(function(r) {
    ...
    return { Recruiter: r, Avg_Accuracy: calcAcc(d.pass, d.count), Audit_Count: d.count, Errors: d.fail, PM: d.pm };
  }).sort(function(a,b){ return a.Avg_Accuracy - b.Avg_Accuracy; });

  // Build pm_stats
  var pm_stats = Object.keys(pmMap).map(function(p) {
    var d = pmMap[p];
    return { PM: p, Avg_Accuracy: calcAcc(d.pass, d.count), Audit_Count: d.count };
  }).sort(function(a,b){ return b.Avg_Accuracy - a.Avg_Accuracy; });

  // Build stage_stats
  var stage_stats = Object.keys(stageMap).map(function(s) {
    var d = stageMap[s];
    var tot = d.count + d.na;
    return { Stage: s, Opportunity_Count: tot, Opportunity_Pass: d.pass,
  ...
  var crit_stats = Object.keys(critMap).map(function(c) {
    var d = critMap[c];
    var tot = d.count + d.na;
    return { Criticality: c, Opportunity_Count: tot, Opportunity_Pass: d.pass,
             Opportunity_Fail: d.fail, Opportunity_NA: d.na, Accuracy: calcAcc(d.pass, d.count) };
  });

  // Build hire type month stats
  ...
  var hireTypeStats = {
    HPE_Experienced: {
      ...
      totals: {
        count: htTotals.HPE_Experienced.count,
        pass:  htTotals.HPE_Experienced.pass,
        fail:  htTotals.HPE_Experienced.fail,
        na:    htTotals.HPE_Experienced.na,
    ...
    HPE_UR: {
      ...
      totals: {
        count: htTotals.HPE_UR.count,
        pass:  htTotals.HPE_UR.pass,
        fail:  htTotals.HPE_UR.fail,
        na:    htTotals.HPE_UR.na,
        accuracy: calcAcc(htTotals.HPE_UR.pass, htTotals.HPE_UR.count)
      }
    }
  };

  // Build PERF_DATA recruiter_monthly with monthly breakdown
  var recruiter_monthly = Object.keys(recruiterMap).map(function(r) {
    var d = recruiterMap[r];
    var mData = {};
    allMonths.forEach(function(m) {
      var md = d.months[m];
      mData[m.toLowerCase()] = md && md.count > 0 ? calcAcc(md.pass, md.count) : null;
    });
    return Object.assign({ name: r, audits: d.count, errors: d.fail, pm: d.pm || '' }, mData);
  });

  // Build PM recruiter mapping
  var pm_recruiters = {};
  Object.keys(pmMap).forEach(function(p) {
    pm_recruiters[p] = Array.from(pmMap[p].recruiters);
  });

  // Build param_weekly (16 weekly slots, 4 months × 4 weeks)
  var param_weekly = {};
  top_errors.forEach(function(pe) {
    var slots = [];
    ['Jan','Feb','Mar','Apr'].forEach(function(m) {
      [1,2,3,4].forEach(function(w) {
        var wk = weekMap[m + '||' + w];
        if (!wk || wk.count === 0) { slots.push(0); return; }
        var pErr = rows.filter(function(r) {
          return (r.Month||'').toLowerCase().slice(0,3) === m.toLowerCase().slice(0,3) &&
                 parseInt(r.Week) === w &&
                 (r.Parameter||'').trim() === pe.Parameter &&
                 classifyResult(r.Result) === 'fail';
        }).length;
        slots.push(wk.count > 0 ? +((pErr/wk.count)*100).toFixed(2) : 0);
      });
    });
...
function _fullDashboardRebuild() {
  ...
  // Rebuild other tabs lazily (they'll also re-init on next visit due to reset flags)
  setTimeout(function() {
    updateTrendCharts();
    updateImprovementCharts();
  }, 100);
...
function _updateDataStatusPanel(result, file) {
  ...
  panel.innerHTML = \`
    ...
    <div class="validation-item">
      <span class="val-icon" style="color:var(--hpe-green)">&#9989;</span>
      <div class="val-text"><strong>Total rows processed:</strong> \${result.rawRowCount.toLocaleString()} audit records</div>
    </div>
    <div class="validation-item">
      <span class="val-icon" style="color:var(--hpe-green)">&#9989;</span>
