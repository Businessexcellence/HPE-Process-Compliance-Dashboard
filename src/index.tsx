...
<head>
  ...
  <style>
    ...
    html.dark .insight-icon.purple { background: rgba(124,58,237,0.15); }
    html.dark .insight-icon.teal   { background: rgba(0,151,167,0.15); }
    ...
    .insight-icon.purple { background: #f3e8ff; color: #7c3aed; }
    .insight-icon.teal   { background: #e0f7fa; color: #0097a7; }
    .insight-item.purple { border-left-color: #7c3aed; }
    .insight-item.teal   { border-left-color: #0097a7; }
...
<div class="content">
  ...
  <div class="tab-content active" id="tab-executive">
    ...
    <!-- Month-wise Summary Table — split Experienced / UR -->
    <div class="card card-full">
      ...
      <div class="card-subtitle">Month-over-month breakdown split by hire type</div>
      <!-- Tab toggle -->
      <div style="display:flex;gap:6px;margin:10px 0 8px 0">
        <button id="monthTabAll" class="filter-btn active" onclick="switchMonthTab('all',this)" style="font-size:12px;padding:5px 14px">All Combined</button>
        <button id="monthTabExp" class="filter-btn" onclick="switchMonthTab('exp',this)" style="font-size:12px;padding:5px 14px;background:#e8f0fb;color:#0D5DBF;border-color:#0D5DBF">HPE Experienced</button>
        <button id="monthTabUR"  class="filter-btn" onclick="switchMonthTab('ur',this)"  style="font-size:12px;padding:5px 14px;background:#fff3e6;color:#FF8300;border-color:#FF8300">HPE UR</button>
      </div>
      <!-- Combined table -->
      <div id="monthTableAll" class="table-container">
        <table>
          <thead><tr>
            <th>Month</th><th>Total Audits</th><th>Passed</th><th>Failed</th><th>N/A</th><th>Accuracy %</th><th>Error %</th><th>Pass %</th><th>MoM Change</th>
          </tr></thead>
          <tbody id="monthSummaryTable"></tbody>
        </table>
      </div>
      <!-- Experienced table -->
      <div id="monthTableExp" class="table-container" style="display:none">
        <table>
          <thead><tr style="background:#e8f0fb">
            <th style="color:#0D5DBF">Month</th><th>Total Audits</th><th>Passed</th><th>Failed</th><th>N/A</th><th>Accuracy %</th><th>Error %</th><th>Pass %</th><th>MoM Change</th>
          </tr></thead>
          <tbody id="monthSummaryExp"></tbody>
        </table>
      </div>
      <!-- UR table -->
      <div id="monthTableUR" class="table-container" style="display:none">
        <table>
          <thead><tr style="background:#fff3e6">
            <th style="color:#FF8300">Month</th><th>Total Audits</th><th>Passed</th><th>Failed</th><th>N/A</th><th>Accuracy %</th><th>Error %</th><th>Pass %</th><th>MoM Change</th>
          </tr></thead>
          <tbody id="monthSummaryUR"></tbody>
        </table>
      </div>
    </div>
  </div>
  ...
  <div class="tab-content" id="tab-trends">
    <div class="section-header">
      ...
      <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap">
        <!-- Hire-type filter — local to this tab -->
        <div style="display:flex;align-items:center;gap:6px">
          <span style="font-size:11px;font-weight:600;color:var(--text-muted);white-space:nowrap"><i class="fas fa-users" style="margin-right:3px"></i>Hire Type:</span>
          <button id="trendHtAll" class="filter-btn active" style="font-size:12px;padding:5px 13px" onclick="applyTrendsHireType('all',this)">All</button>
          <button id="trendHtExp" class="filter-btn" style="font-size:12px;padding:5px 13px;background:#e8f0fb;color:#0D5DBF;border-color:#0D5DBF" onclick="applyTrendsHireType('HPE_Experienced',this)">Experienced</button>
          <button id="trendHtUR"  class="filter-btn" style="font-size:12px;padding:5px 13px;background:#fff3e6;color:#FF8300;border-color:#FF8300"  onclick="applyTrendsHireType('HPE_UR',this)">UR</button>
        </div>
        <!-- Period filter -->
    ...
    <div class="card card-full">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        <div>
          ...
          <div class="card-subtitle">Click any row to see parameter breakdown — <span id="trendsDrillHtLabel" style="font-weight:600;color:var(--hpe-green)">All Hire Types</span></div>
        </div>
      </div>
...
// Helper: render month rows for a given month_stats array
function _buildMonthRows(monthStats) {
  var allSorted = [...monthStats].sort(function(a,b){ return a.Month_Number - b.Month_Number; });
  return allSorted.map(function(m, idx) {
...
function buildMonthTable() {
  // Combined (filter-aware)
  var tbody = document.getElementById('monthSummaryTable');
  if (tbody) tbody.innerHTML = _buildMonthRows(getFilteredMonths().sort(function(a,b){ return a.Month_Number - b.Month_Number; }));

  // HPE Experienced
  var tbodyExp = document.getElementById('monthSummaryExp');
  var expMs = DASHBOARD_DATA.hireTypeStats && DASHBOARD_DATA.hireTypeStats.HPE_Experienced
    ? DASHBOARD_DATA.hireTypeStats.HPE_Experienced.month_stats : [];
  if (tbodyExp) tbodyExp.innerHTML = expMs.length > 0 ? _buildMonthRows(expMs) : '<tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:20px">No HPE Experienced data available</td></tr>';

  // HPE UR
  var tbodyUR = document.getElementById('monthSummaryUR');
  var urMs = DASHBOARD_DATA.hireTypeStats && DASHBOARD_DATA.hireTypeStats.HPE_UR
    ? DASHBOARD_DATA.hireTypeStats.HPE_UR.month_stats : [];
  if (tbodyUR) tbodyUR.innerHTML = urMs.length > 0 ? _buildMonthRows(urMs) : '<tr><td colspan="9" style="text-align:center;color:var(--text-muted);padding:20px">No HPE UR data available</td></tr>';
}

// Tab switcher for Monthly Performance Summary
function switchMonthTab(tab, btn) {
  ['monthTableAll','monthTableExp','monthTableUR'].forEach(function(id){
    var el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
  ['monthTabAll','monthTabExp','monthTabUR'].forEach(function(id){
    var el = document.getElementById(id);
    if (el) el.classList.remove('active');
  });
  var showId = tab === 'exp' ? 'monthTableExp' : tab === 'ur' ? 'monthTableUR' : 'monthTableAll';
  var actId  = tab === 'exp' ? 'monthTabExp'   : tab === 'ur' ? 'monthTabUR'   : 'monthTabAll';
...
function initTrendCharts() {
  ...
  // Populate the Trends week dropdown from TRENDS_HIRE_TYPE data
  populateTrendsWeekOptions();

  // Monthly trend line, weekly error chart, drill table — all via updateTrendCharts
  updateTrendCharts();
...
function buildHeatmap() {
  ...
  if (!container) return;
  // Use trends-tab-local hire type so heatmap reflects the Trends tab filter
  const weekData = getTrendsWeekStats();
  ...
  html += '</div>';
  // Show active hire-type label beneath heatmap when filtered
  if (TRENDS_HIRE_TYPE !== 'all') {
    var htColor = TRENDS_HIRE_TYPE === 'HPE_Experienced' ? '#0D5DBF' : '#FF8300';
    html += '<div style="margin-top:8px;font-size:11px;font-weight:600;color:' + htColor + ';text-align:right">'
      + '<i class="fas fa-filter" style="margin-right:4px"></i>Showing: ' + getTrendsHireTypeLabel() + '</div>';
  }
...
function buildWeeklyTable() {
  ...
  if (!tbody) return;
  // Update the hire-type label shown in the drill table subtitle
  var drillLbl = document.getElementById('trendsDrillHtLabel');
  if (drillLbl) {
    drillLbl.textContent = getTrendsHireTypeLabel();
    drillLbl.style.color = TRENDS_HIRE_TYPE === 'HPE_Experienced' ? '#0D5DBF'
                         : TRENDS_HIRE_TYPE === 'HPE_UR'          ? '#FF8300'
                         : 'var(--hpe-green)';
  }
  // Use Trends-tab-local filtered weeks for rows
  const weeks = getTrendsFilteredWeeks();
  // Use Trends-tab-local full sorted list for WoW baseline (cross-filter accurate)
  const allSorted = [...getTrendsWeekStats()].sort(function(a,b){
    return a.Month_Number !== b.Month_Number ? a.Month_Number - b.Month_Number : a.Week - b.Week;
  });
  tbody.innerHTML = weeks.map(function(w) {
    // Look up previous week from the full sorted list for accurate WoW
...
// ── Trends-tab-local hire type filter ──────────────────────────────────────
// Independent of the global ACTIVE_HIRE_TYPE so toggling it does NOT
// affect the Executive, Improvement, or Insights tabs.
var TRENDS_HIRE_TYPE = 'all'; // 'all' | 'HPE_Experienced' | 'HPE_UR'

function getTrendsMonthStats() {
  if (TRENDS_HIRE_TYPE === 'HPE_Experienced') return DASHBOARD_DATA.hireTypeStats.HPE_Experienced.month_stats;
  if (TRENDS_HIRE_TYPE === 'HPE_UR')          return DASHBOARD_DATA.hireTypeStats.HPE_UR.month_stats;
  return DASHBOARD_DATA.month_stats;
}
function getTrendsWeekStats() {
  if (TRENDS_HIRE_TYPE === 'HPE_Experienced') return DASHBOARD_DATA.hireTypeStats.HPE_Experienced.week_stats;
  if (TRENDS_HIRE_TYPE === 'HPE_UR')          return DASHBOARD_DATA.hireTypeStats.HPE_UR.week_stats;
  return DASHBOARD_DATA.week_stats;
}
function getTrendsHireTypeLabel() {
  if (TRENDS_HIRE_TYPE === 'HPE_Experienced') return 'HPE Experienced';
  if (TRENDS_HIRE_TYPE === 'HPE_UR')          return 'HPE UR';
  return 'All Hire Types';
}

// Helper: get filtered weeks using TRENDS_HIRE_TYPE + current period filter
function getTrendsFilteredWeeks() {
  var all = [...getTrendsWeekStats()].sort(function(a,b){
    return a.Month_Number !== b.Month_Number ? a.Month_Number - b.Month_Number : a.Week - b.Week;
  });
...
// Helper: get filtered months using TRENDS_HIRE_TYPE + current period filter
function getTrendsFilteredMonths() {
  var all = [...getTrendsMonthStats()].sort(function(a,b){ return a.Month_Number - b.Month_Number; });
  if (ACTIVE_FILTER.mode === 'fy' || ACTIVE_FILTER.value === 'all') return all;
  if (ACTIVE_FILTER.mode === 'month') return all.filter(function(m){ return m.Month === ACTIVE_FILTER.value; });
  if (ACTIVE_FILTER.mode === 'week') {
    var ws2 = getTrendsWeekStats();
    var wf  = ws2.find(function(x){ return x.Week_Label === ACTIVE_FILTER.value; });
    return wf ? all.filter(function(m){ return m.Month === wf.Month; }) : all;
  }
  return all;
}

// Apply trends-tab-local hire type and rebuild all Trends tab visuals
function applyTrendsHireType(ht, btn) {
  TRENDS_HIRE_TYPE = ht;
  // Update button active states (Hire Type pill row in Trends tab)
  ['trendHtAll','trendHtExp','trendHtUR'].forEach(function(id){
    var el = document.getElementById(id);
    if (!el) return;
  ...
  if (btn) {
    ...
    if (ht === 'HPE_Experienced') {
      btn.style.background = '#0D5DBF';
      btn.style.color      = 'white';
      btn.style.borderColor= '#0D5DBF';
    } else if (ht === 'HPE_UR') {
      btn.style.background = '#FF8300';
      btn.style.color      = 'white';
      btn.style.borderColor= '#FF8300';
    } else {
      // All — falls back to default .filter-btn.active CSS
      btn.style.background = '';
      btn.style.color      = '';
      btn.style.borderColor= '';
    }
  }
  // Repopulate the Trends week dropdown with hire-type-correct week labels
  populateTrendsWeekOptions();
  // Rebuild all Trends tab visualisations
  if (_trendDone) {
    updateTrendCharts();
    buildHeatmap();
    buildWeeklyTable();
...
// Populate the trendsWeekSelect dropdown using TRENDS_HIRE_TYPE data
function populateTrendsWeekOptions() {
  var sel = document.getElementById('trendsWeekSelect');
  if (!sel) return;
  var weeks = [...getTrendsWeekStats()].sort(function(a,b){
    return a.Month_Number !== b.Month_Number ? a.Month_Number - b.Month_Number : a.Week - b.Week;
  });
  var cur = sel.value;
  var html = '<option value="all">Week</option>';
  weeks.forEach(function(w){
    html += '<option value="' + w.Week_Label + '"' + (w.Week_Label === cur ? ' selected' : '') + '>' + w.Week_Label + '</option>';
  });
  sel.innerHTML = html;
}

// Apply global hire type filter and refresh all charts across all tabs
function applyHireTypeFilter(ht) {
  ...
  populateWeekOptions('execWeekSelect');
  populateTrendsWeekOptions();              // Uses TRENDS_HIRE_TYPE, independent of global HT
  populateWeekOptions('improveWeekSelect');
...
function _buildAuditIntelligenceSnapshot() {
  ...
  // Best week = highest accuracy; Worst week = highest error rate (lowest accuracy)
  ...
  // Top error parameters sorted by error rate %
  var topErrsByRate = [...topErrs].sort(function(a,b){
    var rA = a.Opportunity_Count > 0 ? a.Opportunity_Fail / a.Opportunity_Count : 0;
    var rB = b.Opportunity_Count > 0 ? b.Opportunity_Fail / b.Opportunity_Count : 0;
    return rB - rA;
  });
  var topErr1ByRate = topErrsByRate[0] || null;
  var topErr2ByRate = topErrsByRate[1] || null;

  // Recruiter lists: sorted asc = worst first, desc = best first
  var recSortedAsc  = [...botRec].sort(function(a,b){ return a.Avg_Accuracy - b.Avg_Accuracy; });
  var recSortedDesc = [...botRec].sort(function(a,b){ return b.Avg_Accuracy - a.Avg_Accuracy; });

  // Top performers: top 3 highest accuracy recruiters
  var topRec3 = recSortedDesc.slice(0, 3);

  // Focus recruiters: bottom performers below 97% threshold (up to 3)
  var focusRec = recSortedAsc.filter(function(r){ return r.Avg_Accuracy < 97; }).slice(0, 3);
  ...
  var bestPM    = pmStats.length > 0  ? [...pmStats].sort(function(a,b){ return b.Avg_Accuracy - a.Avg_Accuracy; })[0] : null;
  ...
  function insightItem(type, icon, boldText, bodyText) {
    ...
    // Use explicit icon override if provided, otherwise derive from type
    var ic = icon || faIconMap[type] || 'info-circle';
  ...
  if (aboveTarget >= 0) {
    items.push(insightItem('green', 'trending-up',
      ...
      + (totalFail > 0 ? ' ' + totalFail.toLocaleString() + ' total errors recorded across all parameters.' : ' Zero errors recorded \u2014 perfect compliance.')
    ));
  } else {
    items.push(insightItem('alert', 'exclamation-circle',
      'Overall Accuracy ' + overallAcc + '% \u2014 ' + Math.abs(aboveTarget) + 'pp Below Target:',
      totalCount.toLocaleString() + ' audits processed' + (fyRange ? ' (' + fyRange + ')' : '') + '.'
      + ' Immediate corrective action required to reach 95% target.'
    ));
  }
  ...
  if (sortedMonths.length >= 2 && lastMon && firstMon) {
    ...
    if (!trendUp && !trendFlat && worstMon) {
      items.push(insightItem('warning', 'user-times',
        ...
        'Accuracy moved from ' + firstMon.Accuracy + '% (' + firstMon.Month + ')'
        + ' to ' + lastMon.Accuracy + '% (' + lastMon.Month + ')'
        + ' (' + (momDiff >= 0 ? '+' : '') + momDiff + '%).'
        + (worstMon && worstMon.Accuracy === lastMon.Accuracy
          ? ' ' + lastMon.Month + ' is the lowest-performing month in the period.'
      ...
      items.push(insightItem('green', 'trending-up',
        ...
        + (momDiff >= 0 ? '+' : '') + momDiff + '%).'
        + (bestMon ? ' Peak month: ' + bestMon.Month + ' at ' + bestMon.Accuracy + '%.' : '')
      ));
    }
  }

  // ── 3. Weekly performance extremes — Best & Worst week ──
  if (srcWeeks.length > 0) {
    var bw = bestWeek;
    var ww = worstWeek;
    var sameWeek = bw && ww && bw.Week_Label === ww.Week_Label;
    if (bw && ww && !sameWeek) {
      var wwErrRate = ww.Opportunity_Count > 0
        ? ((ww.Opportunity_Fail / ww.Opportunity_Count) * 100).toFixed(1) : '0';
      var bwErrRate = bw.Opportunity_Count > 0
        ? ((bw.Opportunity_Fail / bw.Opportunity_Count) * 100).toFixed(1) : '0';
      var weekType = ww.Accuracy < 95 ? 'alert' : 'warning';
      items.push(insightItem(weekType, 'fire',
        'Weekly Accuracy Extremes:',
        '\u2b06\ufe0f Best: ' + bw.Week_Label + ' at ' + bw.Accuracy + '% ('
        + bw.Opportunity_Count + ' audits, ' + bwErrRate + '% error rate).'
        + ' \u2b07\ufe0f Worst: ' + ww.Week_Label + ' at ' + ww.Accuracy + '% ('
        + ww.Opportunity_Count + ' audits, ' + wwErrRate + '% error rate, '
        + ww.Opportunity_Fail + ' errors).'
        + (ww.Accuracy < 95 ? ' This week fell below the 95% compliance threshold \u2014 urgent review required.' : '')
      ));
    } else if (bw) {
      // Only one week or identical best/worst
      var bwEr = bw.Opportunity_Count > 0
        ? ((bw.Opportunity_Fail / bw.Opportunity_Count) * 100).toFixed(1) : '0';
      items.push(insightItem('green', 'trending-up',
        'Weekly Performance \u2014 ' + bw.Week_Label + ':',
        bw.Accuracy + '% accuracy across ' + bw.Opportunity_Count + ' audits (' + bwEr + '% error rate).'
      ));
    }
  }

  // ── 4. Parameter with highest error rate ──
  if (topErr1ByRate) {
    var errRate1  = topErr1ByRate.Opportunity_Count > 0
      ? ((topErr1ByRate.Opportunity_Fail / topErr1ByRate.Opportunity_Count) * 100).toFixed(1) : '0';
    var errShare1 = totalErrors > 0
      ? ((topErr1ByRate.Opportunity_Fail / totalErrors) * 100).toFixed(1) : '0';
    var errRate2Str = '';
    if (topErr2ByRate) {
      var errRate2 = topErr2ByRate.Opportunity_Count > 0
        ? ((topErr2ByRate.Opportunity_Fail / topErr2ByRate.Opportunity_Count) * 100).toFixed(1) : '0';
      errRate2Str = ' 2nd: \u201c' + topErr2ByRate.Parameter + '\u201d at ' + errRate2 + '% error rate ('
        + topErr2ByRate.Opportunity_Fail + ' failures).';
    }
    items.push(insightItem('alert', 'exclamation-circle',
      '\u201c' + topErr1ByRate.Parameter + '\u201d \u2014 Highest Error Rate Parameter:',
      errRate1 + '% error rate \u2014 ' + topErr1ByRate.Opportunity_Fail + ' failures out of '
      + topErr1ByRate.Opportunity_Count.toLocaleString() + ' audits.'
      + ' Accounts for ' + errShare1 + '% of all recorded errors.'
      + errRate2Str
      + ' Immediate validation process fix recommended.'
    ));
  }

  // ── 5. Top-performing recruiters ──
  if (topRec3.length > 0) {
    var topNames = topRec3.map(function(r){
      return r.Recruiter + ' (' + r.Avg_Accuracy + '%)';
    }).join(', ');
    var topAudits = topRec3.reduce(function(s,r){ return s + (r.Audit_Count||0); }, 0);
    items.push(insightItem('purple', 'star',
      'Top-Performing Recruiters:',
      topRec3.length + ' recruiter' + (topRec3.length > 1 ? 's' : '') + ' leading the team in compliance accuracy: '
      + topNames + '.'
      + (topAudits > 0 ? ' Combined ' + topAudits.toLocaleString() + ' audits processed at above-average accuracy.' : '')
      + ' Share best practices with the broader team for performance uplift.'
    ));
  }

  // ── 6. Recruiters needing focus — with their PM ──
  if (focusRec.length > 0) {
    var focusLines = focusRec.map(function(r){
      var pmName = r.PM || (function(){
        // Fallback: look up from PERF_DATA.recruiter_monthly by name match
        var match = (PERF_DATA.recruiter_monthly || []).find(function(x){
          return x.name && r.Recruiter && x.name.toLowerCase() === r.Recruiter.toLowerCase();
        });
        return match ? (match.pm || '') : '';
      })();
      return r.Recruiter + ' (' + r.Avg_Accuracy + '%'
        + (r.Audit_Count ? ', ' + r.Audit_Count.toLocaleString() + ' audits' : '')
        + ', ' + (r.Errors || 0) + ' errors'
        + (pmName ? ', PM: ' + pmName : '')
        + ')';
    }).join('; ');
    var pmSet = {};
    focusRec.forEach(function(r){
      var pmName = r.PM || (function(){
        var match = (PERF_DATA.recruiter_monthly || []).find(function(x){
          return x.name && r.Recruiter && x.name.toLowerCase() === r.Recruiter.toLowerCase();
        });
      ...
      if (pmName) pmSet[pmName] = true;
    });
    var pmList = Object.keys(pmSet);
    items.push(insightItem('warning', 'user-times',
      'Recruiters Needing Focused Attention (' + focusRec.length + '):',
      focusLines + '.'
      + (pmList.length > 0
        ? ' Escalate to Program Manager' + (pmList.length > 1 ? 's' : '') + ': '
          + pmList.join(', ') + ' for targeted coaching and process review.'
        : ' Immediate coaching intervention recommended.')
    ));
  }

  // ── 7. Stage accuracy gap ──
  if (stgStats.length >= 2 && worstStg) {
    ...
    if (stgGap > 0.5) {
      items.push(insightItem('info', 'chart-bar',
        ...
        worstStg.Stage + ' at ' + worstStg.Accuracy + '% ('
        + worstStg.Opportunity_Count.toLocaleString() + ' audits, '
        + worstStg.Opportunity_Fail + ' errors) vs '
        + bestStg2.Stage + ' at ' + bestStg2.Accuracy + '%.'
        + ' Focus process improvements on the ' + worstStg.Stage + ' stage.'
      ));
    }
  }

  // ── 8. Experienced vs UR — dedicated accuracy comparison ──
  if (htExp && htUR) {
    var expAcc  = htExp.totals.accuracy;
    var urAcc   = htUR.totals.accuracy;
    var expCnt  = htExp.totals.count  || htExp.totals.total  || 0;
    var urCnt   = htUR.totals.count   || htUR.totals.total   || 0;
    var expFail = htExp.totals.fail   || 0;
    var urFail  = htUR.totals.fail    || 0;
    var htGap2  = +(Math.abs(expAcc - urAcc)).toFixed(2);
    var expErrRate = expCnt > 0 ? ((expFail / expCnt) * 100).toFixed(2) : '0';
    var urErrRate  = urCnt  > 0 ? ((urFail  / urCnt)  * 100).toFixed(2) : '0';
    var higherType = expAcc >= urAcc ? 'HPE Experienced' : 'HPE UR';
    var lowerType  = expAcc >= urAcc ? 'HPE UR' : 'HPE Experienced';
    var higherAcc  = Math.max(expAcc, urAcc);
    var lowerAcc   = Math.min(expAcc, urAcc);
    var itemType2  = htGap2 > 1 ? 'warning' : 'teal';
    items.push(insightItem(itemType2, 'users',
      'Experienced vs UR Accuracy Breakdown:',
      'HPE Experienced: ' + expAcc + '% accuracy ('
      + (expCnt > 0 ? expCnt.toLocaleString() + ' audits, ' : '')
      + expFail + ' errors, ' + expErrRate + '% error rate).'
      + ' HPE UR: ' + urAcc + '% accuracy ('
      + (urCnt > 0 ? urCnt.toLocaleString() + ' audits, ' : '')
      + urFail + ' errors, ' + urErrRate + '% error rate).'
      + (htGap2 > 0.1
        ? ' Gap: ' + htGap2 + 'pp \u2014 ' + higherType + ' leads by ' + htGap2 + 'pp over ' + lowerType + '.'
          + (htGap2 > 1 ? ' Targeted process alignment for ' + lowerType + ' is recommended.' : '')
        : ' Both hire types are performing at near-identical accuracy levels.')
    ));
  }

  // ── 9. Worst PM team ──
  if (worstPM && worstPM.Avg_Accuracy < 99) {
    var pmGap = bestPM && bestPM.PM !== worstPM.PM
      ? +(bestPM.Avg_Accuracy - worstPM.Avg_Accuracy).toFixed(2) : 0;
    items.push(insightItem('info', 'chart-bar',
      worstPM.PM + ' PM Team \u2014 Lowest Team Accuracy:',
      worstPM.Avg_Accuracy + '% accuracy'
      + (worstPM.Audit_Count ? ' across ' + worstPM.Audit_Count.toLocaleString() + ' audits.' : '.')
      + (pmGap > 0 ? ' ' + pmGap + 'pp behind top PM team (' + bestPM.PM + ' at ' + bestPM.Avg_Accuracy + '%).' : '')
      + ' Consider a team process review and coaching session to close the performance gap.'
    ));
  }
...
function updateExecutiveCharts() {
  // Monthly Accuracy Trend (replaces 16-week sparkline)
  var months = getFilteredMonths().sort(function(a,b){ return a.Month_Number - b.Month_Number; });
  var mLabels = months.map(function(m){ return m.Month; });

  destroyChart('sparklineChart');
  var spEl = document.getElementById('sparklineChart');
  if (spEl) {
    var spDatasets;
    var subEl2 = document.getElementById('sparklineSubtitle');
    if (ACTIVE_HIRE_TYPE === 'all') {
      // Experienced + UR + combined lines
      var expMs = [...DASHBOARD_DATA.hireTypeStats.HPE_Experienced.month_stats]
      ...
      var urMs = [...DASHBOARD_DATA.hireTypeStats.HPE_UR.month_stats]
        .sort(function(a,b){ return a.Month_Number - b.Month_Number; })
        .filter(function(m){ return months.some(function(fm){ return fm.Month === m.Month; }); });
      spDatasets = [
        { label: 'Combined', data: months.map(function(m){ return m.Accuracy; }),
          borderColor:'#01A982', backgroundColor:'rgba(1,169,130,0.07)',
          tension:0.4, fill:false, pointRadius:5, borderWidth:2.5,
          pointBackgroundColor: months.map(function(m){ return m.Accuracy<95?'#C54E4B':m.Accuracy<98?'#FF8300':'#01A982'; }),
          pointBorderColor:'white', pointBorderWidth:2 },
        { label: 'HPE Experienced', data: expMs.map(function(m){ return m.Accuracy; }),
          borderColor:'#0D5DBF', backgroundColor:'rgba(13,93,191,0.06)',
          tension:0.4, fill:false, pointRadius:5, borderWidth:2,
          pointBackgroundColor:'#0D5DBF', pointBorderColor:'white', pointBorderWidth:2, borderDash:[4,2] },
        { label: 'HPE UR', data: urMs.map(function(m){ return m.Accuracy; }),
          borderColor:'#FF8300', backgroundColor:'rgba(255,131,0,0.06)',
          tension:0.4, fill:false, pointRadius:5, borderWidth:2,
          pointBackgroundColor:'#FF8300', pointBorderColor:'white', pointBorderWidth:2, borderDash:[2,2] },
        { label: '95% Target', data: mLabels.map(function(){ return 95; }),
          borderColor:'#C54E4B', borderDash:[6,4], borderWidth:1.5, pointRadius:0, fill:false }
      ];
      if (subEl2) subEl2.textContent = 'Monthly accuracy — Experienced vs UR with 95% target line';
    } else {
      var htColor2 = ACTIVE_HIRE_TYPE==='HPE_Experienced' ? '#0D5DBF' : '#FF8300';
      var htBg2    = ACTIVE_HIRE_TYPE==='HPE_Experienced' ? 'rgba(13,93,191,0.1)' : 'rgba(255,131,0,0.1)';
      spDatasets = [
        { label: getHireTypeLabel() + ' Accuracy %', data: months.map(function(m){ return m.Accuracy; }),
          borderColor: htColor2, backgroundColor: htBg2,
          tension:0.4, fill:true, pointRadius:5, borderWidth:2.5,
          pointBackgroundColor: months.map(function(m){ return m.Accuracy<95?'#C54E4B':m.Accuracy<98?'#FF8300':htColor2; }),
          pointBorderColor:'white', pointBorderWidth:2 },
        { label: '95% Target', data: mLabels.map(function(){ return 95; }),
          borderColor:'#C54E4B', borderDash:[6,4], borderWidth:1.5, pointRadius:0, fill:false }
      ];
      if (subEl2) subEl2.textContent = getHireTypeLabel() + ' — monthly accuracy with 95% target line';
    }
    var accVals = months.map(function(m){ return m.Accuracy; });
    var minAcc  = accVals.length > 0 ? Math.min.apply(null, accVals) : 90;
    charts['sparklineChart'] = new Chart(spEl.getContext('2d'), {
      type: 'line',
      data: { labels: mLabels, datasets: spDatasets },
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: {
        ...
        scales: {
          y: { min: Math.max(85, Math.floor(minAcc) - 2), max: 100,
  ...
  // Rebuild month tables (all three: combined, Exp, UR)
  buildMonthTable();
}

// ---- UPDATE ACCURACY TRENDS CHARTS ----
function updateTrendCharts() {
  // Called from initTrendCharts, filter changes, and applyTrendsHireType
  // All data sourced from TRENDS_HIRE_TYPE-aware helpers so this tab
  // is independent of the global ACTIVE_HIRE_TYPE.
  var weeks  = getTrendsFilteredWeeks();
  var months = getTrendsFilteredMonths();

  // ── Monthly trend chart ───────────────────────────────────────────────
  // When TRENDS_HIRE_TYPE === 'all': show Experienced + UR as separate lines
  // When filtered to one type: show single line for that type
  destroyChart('monthlyTrendChart');
  var mtEl = document.getElementById('monthlyTrendChart');
  if (mtEl && months.length > 0) {
    var mtDatasets;
    if (TRENDS_HIRE_TYPE === 'all') {
      // Dual-series: Experienced (blue) + UR (orange) + Combined error rate bar + target
      ...
      // Single-series: one hire type line
      var htColor = TRENDS_HIRE_TYPE === 'HPE_Experienced' ? '#0D5DBF' : '#FF8300';
      var htBg    = TRENDS_HIRE_TYPE === 'HPE_Experienced' ? 'rgba(13,93,191,0.12)' : 'rgba(255,131,0,0.12)';
      mtDatasets = [
        { label: getTrendsHireTypeLabel() + ' Accuracy %', data: months.map(function(m){return m.Accuracy;}),
          borderColor: htColor, backgroundColor: htBg, tension:0.4, fill:true,
          pointRadius:6, pointBackgroundColor: htColor, pointBorderColor:'white', pointBorderWidth:2, yAxisID:'y', order:1 },
        { label: 'Error Rate %', data: months.map(function(m){return m.Error_Rate;}),
          borderColor:'#C54E4B', backgroundColor:'rgba(197,78,75,0.08)', tension:0.4,
          type:'bar', yAxisID:'y2', order:2 },
        { label: '95% Target', data: months.map(function(){return 95;}),
          borderColor:'#94a3b8', borderDash:[8,4], borderWidth:2, pointRadius:0, fill:false, yAxisID:'y', order:0 }
      ];
    }
    var allAccVals = months.map(function(m){return m.Accuracy;});
    var yMin = Math.max(85, Math.floor(Math.min.apply(null, allAccVals)) - 2);
    charts['monthlyTrendChart'] = new Chart(mtEl.getContext('2d'), {
      ...
      options: {
        ...
        scales: {
          y:  { min: yMin, max: 100, position:'left',  ticks:{callback:function(v){return v+'%';}, font:{size:11}} },
  ...
  if (weEl && weeks.length > 0) {
    charts['weeklyErrorChart'] = new Chart(weEl.getContext('2d'), {
      ...
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend:{display:false},
          title: TRENDS_HIRE_TYPE !== 'all' ? {
            display: true, text: getTrendsHireTypeLabel(),
            font:{size:11}, color: TRENDS_HIRE_TYPE === 'HPE_Experienced' ? '#0D5DBF' : '#FF8300', padding:{bottom:4}
          } : {display:false}
        },
        scales: {
  ...
  // ── Drill table + Critical bar chart ─────────────────────────────────
  buildWeeklyTable();
...
function rebuildCriticalBarChart() {
  ...
  // Use trends-local filtered weeks for delta computation
  var fw = getTrendsFilteredWeeks();
...
function _fullDashboardRebuild() {
  ...
  // Reset global hire type toggle to All on new upload
  ...
  // Reset Trends-tab-local hire type to All on new upload
  TRENDS_HIRE_TYPE = 'all';
  ['trendHtAll','trendHtExp','trendHtUR'].forEach(function(id){
    var el = document.getElementById(id);
    if (!el) return;
    el.classList.remove('active');
  ...
  var tAll = document.getElementById('trendHtAll');
...
function runExportPDF() {
  ...
  setTimeout(function() {
    try {
      ...
      // ── helpers ────────────────────────────────────────────────
      var addPage = function() { doc.addPage(); return margin; };
      var hline = function(y, col) {
        doc.setDrawColor(col||'#e5e7eb'); doc.setLineWidth(0.3);
        doc.line(margin, y, PW - margin, y);
        return y + 4;
      };
      var badge = function(x, y, text, bgHex, textHex) {
      ...
      var secHeader = function(doc2, title, icon, y) {
